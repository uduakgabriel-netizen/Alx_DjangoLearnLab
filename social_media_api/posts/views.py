...
from rest_framework.response import Response
from django.shortcuts import get_object_or_404
import django_filters.rest_framework # Import the filter backend

from .models import Post, Comment
from .serializers import PostSerializer, CommentSerializer
from .permissions import IsAuthorOrReadOnly # Our custom permission
from .pagination import CustomPageNumberPagination # Our custom pagination
from .filters import PostFilter # Our custom filter for posts
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    # --- Filtering ---
    # Specify the filter backend to use for this ViewSet
    filter_backends = [django_filters.rest_framework.DjangoFilterBackend]
    # Specify the filter class to use (our custom PostFilter)
    ...
    def comments(self, request, pk=None):
        ...
        Custom action to retrieve all comments for a specific post.
        Accessible at /posts/{pk}/comments/
        This action will also apply pagination.
        """
        post = self.get_object()
        comments = post.comments.all()

        # Manually apply pagination for this custom action
...
class CommentViewSet(viewsets.ModelViewSet):
    ...
    # --- Pagination ---
    pagination_class = CustomPageNumberPagination # Apply pagination to comments as well
